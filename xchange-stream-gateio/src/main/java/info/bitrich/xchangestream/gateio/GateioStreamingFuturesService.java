...
import info.bitrich.xchangestream.gateio.dto.request.payload.ContractIntervalPayload;
...
@Slf4j
public class GateioStreamingFuturesService extends NettyStreamingService<GateioWsNotification> {
  ...
  public GateioStreamingFuturesService(String apiUri, String apiKey, String apiSecret) {
  ...
  private GateioWsRequest getWsRequest(String channelName, Event event, Object... args) {
    ...
    switch (channelName) {
      ...
      // channel requires contract and interval in payload
      case FuturesConfig.FUTURES_KLINES_CHANNEL:
        {
          CurrencyPair contract = (CurrencyPair) ArrayUtils.get(args, 0);
          Duration interval = (Duration) ArrayUtils.get(args, 1);

          Validate.noNullElements(new Object[] {interval, contract});

          payload =
              ContractIntervalPayload.builder()
                  .interval(interval)
                  .contract(contract)
                  .build();
          break;
  ...
  public void messageHandler(String message) {
    ...
    try {
      ...
      if (notification instanceof GateioMultipleUserTradeNotification) {
        ...
        multipleNotification.toSingleNotifications().forEach(this::handleMessage);
      } else if(notification instanceof GateioMultipleKlinesNotification){
        GateioMultipleKlinesNotification klinesNotification = (GateioMultipleKlinesNotification) notification;
        klinesNotification.toSingleNotifications().forEach(this::handleMessage);
      }else{
